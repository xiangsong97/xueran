...
平台不采用“单元测试 → 集成测试 → 系统测试”的单一树状分类来限制测试范围。用户列出的测试类型实际包含多个不同维度，平台应采用**多维测试模型**：一个测试用例可以同时拥有多个标签，一个测试计划快照可以根据变更风险动态组合不同类型的测试。
...
同一个“支付下单”用例可以是集成级测试，也可以属于 API 测试、业务流程测试、数据一致性测试和回归测试。平台应按属性筛选和组合测试计划快照，而不是强行把它放进某一个目录。

#### 3.2.9 变更影响驱动的智能测试编排

本节解决“代码提交后平台到底怎么决定测什么”的问题。平台不直接默认执行某一种传统测试，而是采用“**确定性规则兜底 + AI 变更分析推荐 + 人工策略控制**”的混合决策机制。

**四类对象必须分开**：

| 对象 | 生命周期 | 作用 | 是否因每次提交生成 |
| --- | --- | --- | --- |
| 测试策略（Test Strategy） | 长期配置，可版本化 | 定义不同阶段的质量底线、风险规则、执行预算和审批条件 | 否，持续复用 |
| 编排任务（Orchestration Task） | 单次触发 | 代表一次代码提交、合并请求、部署、定时或人工触发 | 是，一次触发一个 |
| 测试计划快照（Test-plan Snapshot） | 隶属于单次编排任务，生成后冻结 | 记录这次任务最终要执行、推荐、跳过和等待审批的测试集合 | 是，每个任务最多一个当前版本，可保留调整版本 |
| 测试用例/测试套件 | 长期资产，可复用和版本化 | 作为计划快照中的可执行测试内容 | 否，复用已有资产，必要时增量生成 |

因此，“每次代码提交是否创建测试计划”可以更准确地表述为：**每次触发都会创建一个编排任务，并由编排任务生成一份针对本次变更的测试计划快照**。这份快照不是测试人员长期维护的静态计划，而是本次决策的可审计结果；长期稳定的规则放在测试策略中，长期可复用的内容放在用例和测试套件中。

平台默认不是“每次提交固定跑单元测试”，也不是“每次提交固定跑完整单元、集成和系统测试”。它先判断变更影响，再从多个维度组合最小充分测试；开发侧已有的单元测试可以作为被调用的执行资产，平台不强行替代开发团队的单元测试职责。
...
5. 每次选择结果都要记录决策依据，便于复盘 AI 漏测、误测和测试策略调整；测试计划快照一旦开始执行，原则上不得直接覆盖，只能生成调整版本。
...
为了避免 AI 决策不可控，平台需要提供可配置的测试策略中心。策略中心定义“哪些场景必须测、哪些场景可以由 AI 推荐、哪些场景必须人工确认”，并把规则结果与 AI 推荐结果合并为最终测试计划快照。
...
4. 高风险、低置信度或超预算计划进入人工确认；审批调整后生成新的计划快照版本，保留原版本和调整原因。
5. 最终计划快照写入决策记录，供报告、审计和后续复盘使用。
...
| 测试计划快照 | 必测项、AI 推荐项、建议跳过项、人工确认项，以及快照版本号 |
...
> 本平台**不按传统"单元测试 → 集成测试 → 系统测试"的单一顺序组织测试**。这些层级仍然作为测试资产的一个属性保留，但不是平台的唯一目录。下表只是说明各层级在执行侧的落点；真正决定某次任务执行哪些层级的，是 3.2.9 的变更影响分析、3.2.10 的策略规则和最终测试计划快照。

| 传统测试层级 | 平台中的执行落点 | 说明 |
| ------------ | ---------------- | ------- |
| 单元/组件测试 | 开发 CI 中的 Pytest/Unittest 等 + 平台统一结果接入 | 平台可以触发或接收开发侧单元测试结果，但不替代开发团队维护单元测试代码 |
| 集成测试 | 接口多接口串联、服务间契约测试、数据库/消息链路测试 | 验证模块之间的调用、数据和协议是否协同工作 |
| 系统测试 | WebUI E2E、核心业务流程、跨服务场景、部署后健康检查 | 验证完整系统在接近真实使用方式下是否满足业务要求 |
| 端到端测试 | UI/API/消息/数据库组合执行器 | 从用户入口或业务入口贯穿到结果落库，按风险和阶段选择性执行 |
| 验收/回归/冒烟/Sanity | 从已有用例按 `scope`、`trigger_stage` 和风险标签筛选 | 它们描述测试范围或阶段，不是与单元、集成、系统并列的执行层级 |

#### 4.3.2 测试方法覆盖矩阵（全集与平台定位）

> 三个概念层次的区别：**4.2 的"6 种测试方法"**（等价类/边界值/判定表/场景法/错误推测/状态转换）是用例生成的**算法**；**3.2.8 的多维测试属性**（功能/流程/边界/异常/性能/安全等）是测试资产的**结构化标签**；**4.3.1 的映射**是传统测试层级在执行侧的**落点**。三者均非测试方法全集，完整覆盖见下表（● 内置 / ◐ 可扩展 / ○ 一期外；分期以全文【二期】标注为准）。

| 分类                                    | 测试方法                                                     | 平台定位                                                     |
| --------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 按测试层次                              | 单元/组件、集成、系统、端到端测试                             | ● 内置或可接入（开发 CI、接口串联 / WebUI E2E 全链路）       |
|                                         | 回归测试、冒烟测试（P0 用例集）                              | ● 内置（CI 触发 + 分级用例）                                 |
|                                         | 验收测试、Sanity 测试                                        | ◐ 可扩展（需求追踪矩阵 + 人工评审）                          |
| 按测试目标/功能                         | 功能、流程、场景、界面、接口测试                             | ● 内置                                                       |
|                                         | 业务规则、数据测试（CSV/造数）、报表、批处理、工作流         | ◐ 可扩展（判定表 / 数据驱动 / 定时任务）                     |
| 按质量属性                              | 性能（基准/负载/并发/压力）                                  | ◐ 可扩展（压测引擎 TPS/P95/P99，二期落地）                  |
|                                         | 安全（认证/授权/注入/越权/加密/审计等）                      | ◐ 可扩展（渗透 12 模块，仅授权场景，二期落地）              |
|                                         | 容量/稳定性/尖峰/配置、兼容性（多浏览器）、国际化/可访问性/隐私合规、数据一致性 | ◐ 可扩展                                                     |
|                                         | 可靠性/可用性/易用性/可维护性/可移植性、混沌工程、灾备恢复、监控告警、成本能耗 | ○ 一期外（AI 可辅助生成评审清单；自动化执行远期）            |
| 按执行方式                              | 自动化、手工（手动 UI 任务）、半自动化（脚本可介入）         | ● 内置                                                       |
|                                         | 探索性/随机/模糊/猴子测试                                    | ◐ 可扩展（AI 可生成随机/模糊用例）                           |
|                                         | A/B、灰度、金丝雀                                            | ◐ 可扩展（发布验证触发）                                     |
| 大模型/LLM 专项（被测对象为 AI 应用时） | 提示注入、越狱                                               | ◐ 可扩展（与安全模块相通）                                   |
|                                         | 模型效果评估、幻觉、偏见毒性、安全对齐、红队、RAG、工具调用、多轮、长上下文、并发推理、TTFT/TPOT/tokens/s、显存/OOM、量化精度、版本回滚、数据漂移、token 成本、限流降级 | ◐ 可扩展（专项执行器与 AI 质量规则按项目分期接入）          |
| 按被测对象                              | API、UI（Web/APP）                                           | ● 内置                                                       |
|                                         | 数据库（SQL 断言）、中间件                                   | ◐ 可扩展                                                     |
|                                         | 基础设施（环境即代码）                                       | ○ 一期外                                                     |
...
| 测试计划快照生成 | 输出必测项、AI 推荐项、建议跳过项、人工确认项，并给出理由、风险等级、置信度和预计耗时；生成后形成当前任务的冻结版本 |
...
> 首次接入一个被测系统时，平台先建立“系统基线”，再进入日常的增量测试。基线阶段需要全量理解系统，但不代表每个步骤都必须人工阻塞；只有模型置信度不足、文档与代码冲突或涉及高风险业务时才进入人工确认。

```mermaid
flowchart TD
    START(["首次接入被测系统"]) --> SCAN["扫描代码、配置、接口、数据库、消息和部署信息"]
    SCAN --> MODEL["建立候选系统模型<br/>模块·服务·接口·页面·数据链路·业务流程"]
    MODEL --> REQ["生成或导入候选需求与系统说明<br/>有文档则建立版本关联，无文档则代码反推"]
    REQ --> CHECK{"置信度足够且无关键冲突?"}
    CHECK -- "是" --> ASSET
    CHECK -- "否" --> HUMAN["开发/测试确认关键事实<br/>补充需求、裁定冲突、标记高风险范围"]
    HUMAN --> ASSET["按被测对象选择性生成基线资产"]
    ASSET --> API{"存在接口或数据契约?"}
    API -- "是" --> API_DOC["生成接口采集文档与接口实测基线"]
    API -- "否" --> CASES
    API_DOC --> CASES["生成/整理多维测试用例与测试套件"]
    CASES --> SCRIPT["生成或接入自动化脚本<br/>UI·接口·APP·开发侧单元测试结果"]
    SCRIPT --> SNAPSHOT["生成基线测试计划快照<br/>记录全量基线范围、环境、版本和门禁"]
    SNAPSHOT --> EXEC["执行基线测试并采集结果"]
    EXEC --> STORE["保存基线需求、接口、用例、脚本、操作手册、报告和决策记录"]
```

**这一步的目的**：把一个原本“只有代码、文档分散、测试资产不完整”的系统，整理成平台能够持续识别和复用的测试对象。后续代码提交不需要重新做全量扫描，而是基于这份基线做增量分析。

**输入内容**：

- 代码仓库、分支信息、构建配置、部署配置和只读访问凭据；
- 已有需求、接口文档、数据库 Schema、消息主题、历史用例、历史缺陷和执行报告；
- 可用测试环境、测试账号、测试数据和执行节点能力。

**AI 做什么**：

1. 识别服务、模块、接口、页面、数据库表、消息主题和可能的业务流程；
2. 在有需求文档时建立“需求-代码-接口-用例”关联，在没有需求文档时生成候选需求和影响说明；
3. 生成接口、UI、数据、流程、异常和边界等多维测试资产，并给出置信度；
4. 复用已有稳定用例和脚本，只有覆盖缺口或资产失效时才生成新增内容。

**规则做什么**：

- 定义哪些目录、服务、接口和数据表属于高风险范围；
- 定义哪些资产必须经过开发或测试确认；
- 定义基线阶段的最低覆盖范围、允许访问的环境和测试数据边界；
- 判断接口采集文档、接口实测文档、UI 流程或 APP 测试是否适用于当前系统。

**人工什么时候介入**：

- 候选需求与代码、接口或历史行为存在冲突；
- AI 无法确认关键业务规则，或影响范围置信度低于策略阈值；
- 系统涉及权限、支付、订单、数据迁移、隐私等高风险链路；
- 测试账号、测试数据或执行环境需要人工授权。

人工确认不是让人员逐条重写所有 AI 产物，而是确认关键事实和边界。确认后的需求、接口、用例和风险标签会成为后续增量分析的基线。

**阶段输出**：

- 系统基线模型和候选/确认后的需求说明；
- 适用范围内的接口采集文档、接口实测文档、UI/APP 操作手册；
- 带多维标签的测试用例、测试套件和自动化脚本；
- 基线测试计划快照、执行报告和决策审计记录。

#### 5.1.2 代码修改流程（一期主流程）

```mermaid
flowchart TD
    START(["触发事件<br/>代码提交·合并请求·部署·定时·人工"]) --> TASK["创建编排任务<br/>记录触发源、提交版本、项目、环境和操作者"]
    TASK --> CTX["采集多源上下文<br/>Diff·需求·接口·用例·缺陷·日志·环境·策略"]
    CTX --> CLASSIFY["识别变更类型<br/>逻辑·API契约·数据库·UI·配置·权限·AI链路"]
    CLASSIFY --> IMPACT["变更影响与风险分析<br/>调用链·上下游·数据链路·历史缺陷·业务风险"]
    IMPACT --> RULE["匹配测试策略<br/>必测规则·阶段策略·环境限制·执行预算"]
    RULE --> SNAPSHOT["生成测试计划快照<br/>必测项·推荐项·建议跳过项·待审批项·理由·置信度"]
    SNAPSHOT --> GATE{"是否需要人工确认?"}
    GATE -- "否" --> PREP
    GATE -- "是" --> APPROVE["负责人确认、调整或驳回<br/>生成新的快照版本并保留审计记录"]
    APPROVE --> PREP["准备执行资产<br/>复用脚本，必要时增量生成或修订"]
    PREP --> DISPATCH["按测试对象和执行器分发<br/>单元/组件·API·UI·APP·数据·专项"]
    DISPATCH --> EXEC["并发执行、重试、超时控制"]
    EXEC --> COLLECT["采集日志、截图、响应、覆盖率、性能和环境信息"]
    COLLECT --> REPORT["生成报告和质量门禁结果<br/>通过·失败·阻塞·需人工复核"]
    REPORT --> AUDIT["归档编排任务、计划快照、决策解释和执行记录"]

    CLASSIFY -. "仅接口契约变化时" .-> API_DOC["更新接口影响文档/契约测试"]
    CLASSIFY -. "仅数据库或消息变化时" .-> DATA_PLAN["生成迁移、回滚和数据一致性检查"]
    CLASSIFY -. "仅UI或路由变化时" .-> UI_PLAN["生成页面影响和关键路径回归"]
    CLASSIFY -. "Prompt/RAG/模型/工具变化时" .-> AI_PLAN["生成AI效果、安全和推理性能测试"]
    API_DOC -.-> IMPACT
    DATA_PLAN -.-> IMPACT
    UI_PLAN -.-> IMPACT
    AI_PLAN -.-> IMPACT
...
这张图表达的是日常运行的核心逻辑：**代码提交不是直接等于某一种测试，而是先产生一次编排任务，再由这次任务生成测试计划快照**。计划快照把“为什么选择这些测试”固定下来，之后执行器只按快照执行，避免执行过程中因为 AI 再次判断而产生不可追溯的范围变化。

**步骤 01：触发事件**

- 输入：代码提交、合并请求、部署事件、定时任务或人工触发；
- 平台动作：记录项目、分支、提交版本、触发人、目标环境和触发阶段；
- 输出：一个唯一的 `task_id`；
- 人工参与：普通代码提交不需要人工参与，人工任务可以主动指定范围。

**步骤 02：创建编排任务**

- 编排任务是一次测试决策和执行的容器，不是长期测试计划；
- 同一个提交如果分别在测试环境和预发环境触发，可以产生两个不同编排任务，因为环境、策略和门禁可能不同；
- 任务从创建开始就记录状态，支持排队、暂停、重试、取消和审计。

**步骤 03：采集多源上下文**

- 输入：Git Diff、提交说明、需求版本、接口定义、测试用例、历史缺陷、历史失败记录、环境和执行器状态；
- 平台动作：把分散信息统一关联到当前 `task_id`，并标明来源、版本和时间；
- 输出：本次决策的上下文快照；
- 人工参与：只有缺少权限或数据来源冲突时介入，不要求测试人员手工整理全部资料。

**步骤 04：识别变更类型**

平台先用确定性信号识别变更，再让 AI 补充语义判断。示例：

| 变更信号 | 典型判断 | 后续重点 |
| --- | --- | --- |
| `*.py`、`*.java` 等内部逻辑文件 | 内部逻辑/组件变更 | 受影响组件、边界、异常和历史缺陷回归 |
| OpenAPI、路由、DTO、序列化结构 | API/接口契约变更 | 契约、上下游兼容性和集成流程 |
| SQL、迁移脚本、消息 Schema | 数据/消息链路变更 | 迁移、回滚、数据一致性和消费者 |
| 页面、路由、权限组件 | UI/权限变更 | UI 冒烟、权限、关键用户路径 |
| Prompt、RAG、模型配置、工具注册 | AI 链路变更 | 效果、幻觉、提示注入、工具调用和推理性能 |
| 配置、镜像、部署脚本、基础设施 | 配置/部署变更 | 启动、健康检查、兼容性和回滚 |

- 输出：一个或多个变更类型标签；
- 人工参与：当多个类型冲突或 AI 置信度低于阈值时进入确认。

**步骤 05：变更影响与风险分析**

这一步不是让 AI 凭经验猜测，而是把多种证据合并起来：

1. 根据 Diff 找到直接修改的文件、类、函数、接口、页面和配置；
2. 根据静态依赖图、调用链和服务边界寻找直接和间接受影响对象；
3. 根据接口契约、数据库 Schema、消息主题和配置关系判断数据链路；
4. 根据需求关联、历史缺陷和历史失败记录补充业务风险；
5. 综合风险标签、影响范围和证据完整度，输出影响对象、风险等级和置信度。

输出会区分“已确认影响”“可能影响”和“未发现影响”，并保留证据链接。高风险但高置信度仍然可能需要人工审批；低风险但低置信度也不能直接扩大或缩小范围。

**步骤 06：策略规则匹配**

规则匹配解决的是“公司规定最低必须做什么”，不是让 AI 自由决定一切。系统按以下顺序匹配：

1. 按项目、业务线和触发阶段加载有效测试策略；
2. 按路径、文件类型、变更类型和风险标签匹配强制必测规则；
3. 根据环境策略过滤当前环境不可执行或不允许执行的测试；
4. 根据执行预算限制最大耗时、用例数量、并发数和高成本测试；
5. 把规则结果交给 AI 推荐模块补充可能遗漏的测试。

如果规则明确要求某项安全、权限、迁移或核心链路测试，AI 只能增加测试，不能将其删除。

**步骤 07：生成测试计划快照**

快照至少包含：

- 本次编排任务、代码版本、需求版本和环境版本；
- 必测项、AI 推荐项、建议跳过项和待人工确认项；
- 每项测试的用例/套件 ID、测试维度、执行器、依赖和预计耗时；
- 选择或跳过理由、风险等级、置信度、预算裁剪结果；
- 当前策略版本和生成时间。

生成后快照进入冻结状态。若负责人调整范围，不覆盖原快照，而是生成 `v2` 等新版本，并记录“谁在什么时间改了什么、为什么改”。

**步骤 08：人工门禁**

- 低风险且高置信度：自动执行必测项和预算内推荐项；
- 中风险：按项目策略自动执行，或只对扩展推荐项等待确认；
- 高风险、低置信度、超预算、破坏性测试、生产近似压测和正式验收：进入人工确认；
- 审批人可以批准、缩小范围、扩大范围、退回补充信息或驳回执行。

人工确认确认的是“本次计划快照是否可以执行”，不是要求人工重新编写一套测试。

**步骤 09：准备执行资产**

平台优先复用已有稳定脚本和测试数据；只有以下情况才调用 AI 生成或修订：
...
生成后的脚本仍然需要通过语法检查、静态检查和最小可执行验证。

**步骤 10：执行分发**

执行调度器按计划快照中的测试资产标签选择执行器：

- 开发侧单元/组件测试接入 CI 或对应语言执行器；
- API、契约、数据和消息测试分发到接口执行节点；
- WebUI 测试分发到 Playwright 浏览器节点；
- APP 测试分发到 Appium 设备节点；
- 性能、安全和其他专项测试只在策略允许的环境执行。

执行器不重新改变计划范围，只负责处理依赖、并发、重试、超时和资源隔离。

**步骤 11：结果采集**

平台统一采集通过、失败、阻塞、跳过和未执行状态，以及日志、截图、视频、响应、数据库断言、覆盖率、性能指标和环境信息。失败结果需要关联到具体用例、脚本、变更对象和计划快照。
...
1. 测试结果如何，是否通过质量门禁；
2. 为什么执行这些测试，哪些测试没有执行，是否经过人工调整。

报告中展示影响范围、策略命中、AI 推荐理由、置信度、执行成本、审批记录和结果。这样即使测试失败或发生漏测，也能定位是变更识别、影响分析、规则配置、脚本执行还是环境问题。

#### 5.1.3 二期整体流程

二期不改变一期的主链路，而是在执行后增加“结果理解、缺陷闭环、资产沉淀和策略优化”。AI 的自迭代必须建立在审核后的样本上，不能把未经确认的失败结果直接写入规则或 Prompt。

```mermaid
flowchart TD
    START["沿用一期编排任务与测试计划快照"] --> EXEC["执行测试并采集全量证据"]
    EXEC --> FILTER["误报过滤与失败聚类<br/>区分环境、脚本、依赖和真实缺陷"]
    FILTER --> ROOT["根因分析<br/>调用链·堆栈·日志·变更对象"]
    ROOT --> REVIEW{"是否确认是真实缺陷?"}
    REVIEW -- "是" --> DEFECT["创建分级缺陷并推送责任人"]
    REVIEW -- "否/待确认" --> MANUAL["人工复核并补充结论"]
    DEFECT --> ASSET["沉淀测试资产<br/>用例·脚本·数据·报告·缺陷关联"]
    MANUAL --> ASSET
    ASSET --> REPORT["形成质量趋势和编排效果报告"]
    REPORT --> OPT["生成策略、风险标签和Prompt优化建议"]
    OPT --> APPROVE["负责人审核并版本化发布<br/>支持回滚"]
    APPROVE --> NEXT["后续任务使用新版本策略/Prompt"]
```

**二期新增的闭环说明**：

- **误报过滤**：结合环境状态、依赖服务、历史失败模式和重复堆栈，先判断失败是否可能由环境或脚本造成；
- **根因定位**：把失败证据与变更对象、调用链、日志和堆栈关联，输出候选根因，不直接替代人工定责；
- **缺陷流转**：只有经过规则或人工确认的真实缺陷才创建工单，并带上复现证据、影响范围和关联提交；
- **资产沉淀**：把有效用例、稳定脚本、数据构造方式、失败模式和缺陷关联回测试资产库；
- **策略与 Prompt 优化**：系统生成建议，负责人审核后才形成新版本；新版本支持灰度、效果对比和回滚；
- **效果复盘**：统计推荐命中率、漏测、误报、计划耗时、强制项执行率和人工调整原因，用于判断 AI 是否真的变好。

### 5.2 代码提交后的自动编排策略

代码提交后，平台默认不固定执行某一个传统测试类型，而是按触发阶段、风险等级和执行成本，为本次编排任务生成一份测试计划快照。不同阶段不是简单地把“单元、集成、系统”串成固定流水线，而是改变必测规则、允许的成本和人工门禁。

| 触发阶段 | 默认策略 | 该阶段测试计划快照通常包含 | 人工确认条件 |
| --- | --- | --- | --- |
...
- 代码提交阶段默认追求快速反馈，因此通常生成受影响范围内的静态检查、组件测试、接口/页面冒烟和轻量回归快照；如果开发侧没有接入单元测试，平台不能凭空执行不存在的单元测试，而是记录“未接入”并由策略决定是否阻断；
- 合并请求阶段重点验证变更能否与上下游协同工作，因此会增加契约测试、受影响集成测试和关键路径回归；
- 合并后或部署后阶段重点验证环境和部署结果，因此会增加健康检查、部署后冒烟、迁移校验和回滚验证；
- 发布前阶段才适合扩大到核心系统级端到端、关键安全、性能基线和正式验收；
- 测试人员可以手动触发任意范围，但系统仍会先用策略检查环境、权限、成本和审批条件。

所以，平台的默认机制是“**阶段策略提供底线，变更影响决定范围，AI 补充推荐，人工审批控制例外**”。不是把所有测试层级都在每次提交时完整跑一遍，也不是把所有选择都交给 AI。
