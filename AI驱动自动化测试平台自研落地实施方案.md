...
- **规则兜底 + AI 推荐 + 人工介入**：强制必测项由规则引擎保障，AI 负责扩展推荐与理由说明，高风险/低置信度场景标记为需人工介入
...
flowchart TB
    ...
    subgraph L2["02 业务层"]
        ...
        PLAN["编排任务与测试计划快照服务<br/>必测项·推荐项·人工介入项"]
...
- **需求来源判断**：系统自动检测任务是否关联最新的需求文档：
    ...
    - **无文档** → 代码反推需求与影响说明，作为候选测试依据；高风险或低置信度时标记为需人工介入，普通变更自动继续
...
| 测试计划快照 | 输出必测、推荐、跳过和待介入项，保存理由、置信度、成本和版本 |
| 介入与执行交接 | 普通测试自动执行；高风险或低置信度结果标记为建议人工复核；高影响测试获得人工授权后，将冻结快照交给 4.3 执行模块 |
...
| 5 | **多维测试属性模型** | 全新开发 | — | 用例、脚本、任务、报告增加测试层级、被测对象、测试目标、质量属性、执行方式、测试范围、触发阶段、风险标签、数据与环境画像、资产状态等字段 | **P0** |
...
| 任务 | 编排任务 / 任务触发记录 / 测试计划快照 / 编排决策记录 / 人工介入记录 / 执行记录 | N-Tester + 新增触发源、代码版本、环境、影响范围、快照版本、必测项、推荐项、跳过项、人工介入项与执行策略字段 |
...
| 4 | 代码反推需求与影响范围准确性不足 | 中 | 中 | 高风险/低置信度标记为需人工介入；知识库自迭代持续优化提示词 |
...
| 12 | AI 错判变更影响范围导致漏测 | 中 | 高 | 关键模块配置强制回归规则；高风险/低置信度计划标记为需人工介入；保留决策依据用于复盘 |
