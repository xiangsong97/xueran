...
| 开发反馈 | 在提交或合并请求阶段获得测试结果，不必等到后期集中测试才发现问题 | 缩短反馈周期，降低问题积累带来的交付风险 |
| 测试人员 | AI 自动完成接口、UI、APP、冒烟、回归和基础数据校验，缓解系统多、功能多、人工测不完的问题 | 聚焦核心功能、复杂场景和人工探索测试 |
| 测试覆盖 | 根据变更影响自动选择相关模块、接口、流程和用例，减少人工遗漏 | 逐步形成更稳定、可复用的回归范围 |
| 测试效率 | 多个测试任务可以并行执行，减少用例准备、脚本编写和重复回归时间 | 提升有限测试资源的利用率 |
...
见 3.2.3 自迭代流程 + 第 5.1.3 节。核心：把"执行结果/缺陷数据/用例变更"作为样本输入，反向优化 Agent 提示词与用例库，形成**测试资产持续增值**的飞轮。
...
业务优先级 P0-P3 不属于一期测试资产模型和测试计划生成条件。一期通过风险标签、变更类型、影响范围和强制测试规则确定测试范围；二期由 4.9 优先级管理与分级响应模块统一管理 P0-P3，并用于测试范围升级、通知、人工复核和质量门禁。
...
```mermaid
flowchart LR
    A["一期测试结果<br/>日志·截图·Trace·人工复核记录"] --> B["结果采集与标准化"]
    B --> C["失败聚类与误报过滤"]
    C --> D["根因分析与影响定位"]
    D --> E["缺陷创建、分级与通知"]
    E --> F["用例·脚本·规则·测试数据·知识库资产沉淀"]
    F --> G["质量趋势、漏测与推荐命中率复盘"]
    G --> H["生成策略、Prompt、用例与优先级优化建议"]
    H --> I{"人工审核与灰度验证"}
    I -- "通过" --> J["策略·Prompt·知识库·优先级规则版本化更新"]
    I -- "不通过" --> K["保留当前版本并记录驳回原因"]
    J --> L["反哺下一轮测试计划生成与执行"]
```

二期在一期测试结果之上增加结果理解、缺陷闭环、资产沉淀和智能优化能力，不改变一期的测试计划生成和测试环境执行主链路。平台定时读取一期执行结果、日志、截图、Trace、人工复核记录、缺陷记录和用例变更记录，生成策略、Prompt、用例和优先级规则优化建议。所有优化结果必须经过人工审核、灰度验证和版本化管理，确认后才反哺后续测试计划，不涉及生产环境部署。

### 5.2 三类触发场景的执行策略

| 触发场景 | 默认测试范围 | 主要目的 | 说明 |
| --- | --- | --- | --- |
| 代码提交或合并请求 | 受影响组件、接口/页面冒烟、相关集成和必要回归 | 快速反馈 | 自动读取 Git Diff；测试环境部署完成后可继续触发健康检查和冒烟验证 |
...
5.2 只说明三类触发方式下的默认执行范围，不重新定义 3.2.10 的策略匹配算法。无论哪种触发方式，平台都先生成测试计划快照，再按快照在测试环境执行；代码提交不会固定等于单元测试，也不会每次执行完整系统测试。平台只输出测试报告、测试证据和质量结论，不连接生产环境，也没有部署、推送或回滚生产代码的功能和权限。
...
| 智能增强建设<span style="color:#e8590c">【二期】</span> | 3 个月 | 知识库自迭代、根因定位与误报过滤、缺陷闭环、P0-P3 优先级管理与分级响应、优先级驱动通知和测试范围扩展、压测/安全框架增强 | 完整智能闭环能力 | 优化建议可审核灰度生效，复杂专项测试和优先级能力可控启用 |
...
| 19 | 二期优先级规则配置不合理，导致测试范围扩大或通知过多 | 中 | 中 | 优先级规则版本化；先在少量系统灰度验证，复盘范围命中率、通知数量和人工复核结果后再推广 |
